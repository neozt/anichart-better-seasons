...
(function () {
  ...
  function replaceSeasonLinks() {
    ...
    console.log(
      `[anichart-better-seasons] currentSeason: ${JSON.stringify(currentSeason)}, currentUrl: ${currentUrl}`,
    );
    ...
    const seasonLinks = [-2, -1, 0, 1, 2].map((i) =>
  ...
  function handleKeyboardShortcuts(event) {
    ...
    if (
      event.key === CONFIG.previousSeasonKey &&
      (CONFIG.previousSeasonKeyMod === undefined ||
        event[CONFIG.previousSeasonKeyMod])
    ) {
      const previousSeason = incrementSeason(currentSeason, -1);
      window.location.assign(constructSeasonUrl(previousSeason));
    } else if (
      event.key === CONFIG.nextSeasonKey &&
      (CONFIG.nextSeasonKeyMod === undefined || event[CONFIG.nextSeasonKeyMod])
    ) {
  ...
  function createSeasonLink(document, { season, dataAttr, activeLink }) {
    ...
    anchor.className = activeLink
      ? "season router-link-exact-active router-link-active"
      : "season";
